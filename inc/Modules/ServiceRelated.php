...
class ServiceRelated {
	public static function rt_service_related() {
		...
		$related_item_number = kariez_option( 'rt_service_related_limit' );
		...
		$query_type = kariez_option( 'rt_service_related_query' );
		...
		if( kariez_option( 'rt_service_related_sort' ) ){

			$post_order = kariez_option( 'rt_service_related_sort' );
		...
		if( $related_query->have_posts() && kariez_option( 'rt_service_related' ) ) { ?>
			<div class="rt-service-default rt-service-multi-layout-1 rt-related-service">
				<div class="container">
					<h2 class="related-title"><?php kariez_html( kariez_option( 'rt_service_related_title' ) , 'allow_title' );?></h2>
									...
									<div class="service-thumbs">
										<?php if (!empty( $rt_service_icon )  ) { ?>
											<div class="service-icon">
												<i <?php echo wp_specialchars_decode( esc_attr( $service_bg ), ENT_COMPAT ); ?> class="<?php kariez_html( $rt_service_icon , false );?>"></i>
											</div>
										<?php } else  {

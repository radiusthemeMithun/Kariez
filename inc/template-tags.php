...
use RT\Kariez\Modules\PostShare;

/*Allow HTML for the kses post*/
function kariez_html( $html, $context = '' ) {
...
if ( ! function_exists( 'kariez_custom_menu_cb' ) ) {
	/**
	 ...
	 * @return string|void
	 */
	function kariez_custom_menu_cb( $args ) {
		extract( $args );
		$add_menu_link = admin_url( 'nav-menus.php' );
		$menu_text     = sprintf( __( "Add %s Menu", "kariez" ), $theme_location );
		__( 'Add a menu', 'kariez' );
		if ( ! current_user_can( 'manage_options' ) ) {
			$add_menu_link = home_url();
			$menu_text     = __( 'Home', 'kariez' );
		}
...
if ( ! function_exists( 'kariez_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function kariez_menu_icons_group( $args = [] ) {
		$default_args = [
			...
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				...
				<?php if ( $args['search'] ) : ?>
					<li class="rt-search-popup">
						<a class="menu-search-bar rt-search-trigger" href="#header-search" aria-label="search popup"><i class="icon-search"></i></a>
					</li>
				<?php endif; ?>
...
if ( ! function_exists( 'kariez_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function kariez_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'kariez_readmore_text' ) ) {
	/**
	 * Creates continue reading text.
	 *
	 * @return string
	 */
	function kariez_readmore_text() {

		if ( empty( kariez_option( 'rt_blog_read_more' ) ) ) {
			return;
		}
		return sprintf(
			'%s %s',
			esc_html( kariez_option( 'rt_blog_read_more' ) ),
			the_title( '<span class="screen-reader-text">', '</span>', false )
		);
	}
}

if ( ! function_exists( 'kariez_list_item_separator' ) ) {
	/**
	 ...
	 * @since 6.0.0
	 */
	function kariez_list_item_separator() {
...
if ( ! function_exists( 'kariez_posted_in' ) ) {
	/**
	 * Prints HTML with category list information about theme categories.
	 * @return string
	 */
	function kariez_posted_in( $type = 'category' ) {
		$categories_list = get_the_category_list( kariez_list_item_separator() );
		if ( 'tag' === $type ) {
			$categories_list = get_the_tag_list( '', kariez_list_item_separator() );
		}
		if ( $categories_list ) {
			return sprintf(
...
if ( ! function_exists( 'kariez_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 * @return string
	 */
	function kariez_posted_on() {
		$time_string = sprintf(
...
if ( ! function_exists( 'kariez_posted_by' ) ) {
	/**
	 * Prints HTML with meta information about theme author.
	 * @return string
	 */
	function kariez_posted_by( $prefix = '' ) {
		return sprintf(
			esc_html__( '%s %s', 'kariez' ),
...
if ( ! function_exists( 'kariez_get_svg' ) ) {
	/**
	 * Get svg icon
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function kariez_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'kariez_svg_icon_list', [
...
if ( ! function_exists( 'kariez_get_file' ) ) {
	/**
	 * Get File Path
	 *
	 * @param $path
	 ...
	 * @return string
	 */
	function kariez_get_file( $path, $return_path = false ): string {
...
if ( ! function_exists( 'kariez_get_img' ) ) {
	/**
	 * Get Image Path
	 *
	 * @param $filename
	 ...
	 * @return string|void
	 */
	function kariez_get_img( $filename, $echo = false, $image_meta = '' ) {
		$path      = '/assets/images/' . $filename;
		$image_url = kariez_get_file( $path );

		if ( $echo ) {
			if ( ! strpos( $filename, '.svg' ) ) {
				$getimagesize = wp_getimagesize( kariez_get_file( $path, true ) );
...
if ( ! function_exists( 'kariez_get_css' ) ) {
	/**
	 * Get CSS Path
	 *
	 * @param $filename
...
if ( ! function_exists( 'kariez_get_js' ) ) {
	 ...
	 * @return string
	 */
	function kariez_get_js( $filename ) {
...
if ( ! function_exists( 'kariez_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function kariez_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'kariez_get_social_html' ) ) {
	/**
	 * Get Social markup
	 *
	 * @param $color
	...
	function kariez_get_social_html( $color = '' ) {
		ob_start();
...
if ( ! function_exists( 'kariez_site_logo' ) ) {
	 ...
	 *
	 */
	function kariez_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = kariez_option( 'rt_logo' );
		$logo_light      = kariez_option( 'rt_logo_light' );
		$logo_mobile     = kariez_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'kariez_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function kariez_footer_logo() {
		$main_logo  = kariez_option( 'rt_logo' );
		$logo_light = kariez_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'kariez_scroll_top' ) ) {
	/**
	 * Back-to-top button
	 * @return void
	 */
	function kariez_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( kariez_option( 'rt_back_to_top' ) ) {
			?>
...
if ( ! function_exists( 'kariez_meta_icons' ) ) {
	/**
	 * Get meta icons
	 *
	 * @param $name
...
if ( ! function_exists( 'kariez_reading_time' ) ) {
	...
	function kariez_reading_time() {
		...
		if ( $reading_time < 1 ) {
			$result = esc_html__( 'Less than a minute', 'kariez' );
		} elseif ( $reading_time > 60 ) {
			$result = sprintf( esc_html__( '%s hours read', 'kariez' ), floor( $reading_time / 60 ) );
		} else if ( $reading_time == 1 ) {
			$result = esc_html__( '1 min read', 'kariez' );
		} else {
			$result = sprintf( esc_html__( '%s mins read', 'kariez' ), $reading_time );
		}
...
if( !function_exists( 'rt_post_views' )){
	...
	function rt_post_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$kariez_view_html = esc_html__( 'View' , 'kariez' );
		} else {
			$kariez_view_html = esc_html__( 'Views' , 'kariez' );
		}

		$kariez_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $kariez_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $kariez_views_html.'</span> ';
	}
}

if ( ! function_exists( 'kariez_post_meta' ) ) {
	/**
	 * Get post meta
	 *
	 * @return string
...
//			'with_icon'     => false,
		...
		$comments_text   = sprintf( _n( 'Comment: %s', 'Comments: %s', $comments_number, 'kariez' ), number_format_i18n( $comments_number ) );

		$_meta_data = [];
		$output     = '';

		$_meta_data['author']   = kariez_posted_by( $args['author_prefix'] );
		$_meta_data['date']     = kariez_posted_on();
		$_meta_data['category'] = kariez_posted_in();
		$_meta_data['tag']      = kariez_posted_in( 'tag' );
		$_meta_data['comment']  = esc_html( $comments_text );
		$_meta_data['reading']  = kariez_reading_time();
...
if ( ! function_exists( 'kariez_post_thumbnail' ) ) {
	/**
	 * Displays post thumbnail.
	 * @return void
	 */
	function kariez_post_thumbnail( $size = 'full', $thumb_date = false ) {
		...
		<?php } else { ?>
			...
			if ( kariez_option( 'rt_video_visibility' ) == 1 && ( 'video' == get_post_format( get_the_ID() ) ) && !empty( $rt_youtube_link ) ) { ?>
				<div class="rt-video">
					<a class="popup-youtube video-popup-icon" href="<?php echo esc_url( $rt_youtube_link );?>"><i class="icon-play"></i></a>
				</div>
			<?php } ?>
		<?php } ?>
...
if ( ! function_exists( 'kariez_post_single_thumbnail' ) ) {
	/**
	 * Display post details thumbnail
	 * @return void
	 */
	function kariez_post_single_thumbnail( $size = 'full' ) {
		if ( ! Fns::can_show_post_thumbnail() ) {
			return;
		}
		?>
		...
		<?php } else { ?>
			...
			<?php if ( wp_get_attachment_caption( get_post_thumbnail_id() ) && kariez_option( 'rt_single_caption_visibility' ) == 1 ) : ?>
				<figcaption class="wp-caption-text">
					<span><?php kariez_html( wp_get_attachment_caption( get_post_thumbnail_id() ) ); ?></span>
				</figcaption>
			<?php endif; ?>
		<?php } ?>
		</div>
...
if ( ! function_exists( 'kariez_entry_footer' ) ) {
	/**
	 * Kariez Entry Footer
	 *
	 * @return void
	...
	function kariez_entry_footer() {
		if ( ! is_single() ) {
			...
			if ( ( has_tag() && kariez_option( 'rt_single_tag_visibility' ) ) || kariez_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( kariez_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = kariez_option( 'rt_tags' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>

							<?php
							kariez_single_post_footer_meta(
								'content-below-meta', [ 'tag' ]
							);
							?>
						</div>
					<?php }
					if ( kariez_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = kariez_option( 'rt_social' ) ) {

							} ?>
							<?php PostShare::kariez_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'kariez_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function kariez_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'kariez_entry_profile' ) ) {
	/**
	 * Kariez Entry Profile
	 *
	 * @return void
	 *
	 */
	function kariez_entry_profile() {
		if ( kariez_option( 'rt_single_profile_visibility' ) ) {
					...
					<div class="profile-content">
						<div class="profile-author-info">
							...
							<div class="profile-info">
								...
								<?php if ( $prof_phone ) { ?>
									<span class="profile-phone"><?php echo esc_html__( 'Ph: ', 'kariez' ) ?><?php echo esc_html( $prof_phone );?></span>
								<?php } ?>
							</div>
						</div>
...
if ( ! function_exists( 'kariez_single_post_footer_meta' ) ) {
	/**
	 * Get single post footer meta
	 *
	 * @return string
	 */
	function kariez_single_post_footer_meta( $class = '', $includes = [ 'tag' ] ) {
		if ( is_single() && kariez_option( 'rt_single_tag_visibility' ) ) : ?>
			<div class="post-footer-meta <?php echo esc_attr( $class ) ?>">
				<?php echo kariez_post_meta( [
					'with_list' => false,
					'with_icon' => false,
					'include'   => $includes,
				] ); ?>
...
if ( ! function_exists( 'kariez_entry_content' ) ) {
	/**
	 * Entry Content
	 * @return void
	 */
	function kariez_entry_content() {
		if ( ! is_single() ) {
			$length = kariez_option( 'rt_excerpt_limit' );
...
if ( ! function_exists( 'kariez_sidebar' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function kariez_sidebar( $sidebar_id ) {
		...
		?>
		<aside id="sidebar" class="kariez-widget-area sidebar-sticky <?php echo esc_attr( $sidebar_cols ) ?>"
			   role="complementary">
...
if ( ! function_exists( 'kariez_post_class' ) ) {
	/**
	 * Get dynamic article classes
	 * @return string
	 */
	function kariez_post_class( $default_class = 'kariez-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . kariez_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= kariez_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = kariez_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= kariez_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = kariez_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . kariez_option( 'rt_blog_style' );
...
if ( ! function_exists( 'kariez_separate_meta' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function kariez_separate_meta( $class = '', $includes = [ 'category' ] ) {
		if ( ( ! is_single() && kariez_option( 'rt_blog_above_meta_visibility' ) ) || ( is_single() && kariez_option( 'rt_single_above_meta_visibility' ) ) ) : ?>
...
if ( ! function_exists( 'kariez_single_entry_header' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function kariez_single_entry_header() {
		?>
		<header class="entry-header">
			...
			if ( ! empty( Fns::single_meta_lists() ) && kariez_option( 'rt_single_meta_visibility' ) ) {
...
if ( ! function_exists( 'kariez_breadcrumb' ) ) {
	/**
	 * Kariez breadcrumb
	 * @return void
	 */
	function kariez_breadcrumb() {
		?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>"><i class="icon-home"></i><?php esc_html_e( 'Home', 'kariez' ) ?></a>
					<span class="raquo"></span>
				</li>
				<li class="breadcrumb-item active" aria-current="page">
					<?php
					if ( is_tag() ) {
						esc_html_e( 'Posts Tagged: ', 'kariez' );
						?>
						...
						esc_html_e( 'Posts made in: ', 'kariez' );
						echo esc_html( get_the_time( is_year() ? 'Y' : ( is_month() ? 'F, Y' : 'F jS, Y' ) ) );
						echo '</span>';
					} elseif ( is_search() ) {
						echo '<span class="title">';
						esc_html_e( 'Search results for: ', 'kariez' );
						the_search_query();
						echo '</span>';
					} elseif ( is_404() ) {
						echo '<span class="title">';
						esc_html_e( '404', 'kariez' );
						echo '</span>';
					} elseif ( is_single() ) {
						...
						esc_html_e( 'Posts Category: ', 'kariez' );
						echo '<span class="title">';
						single_cat_title();
						echo '</span>';
					} elseif ( is_tax() ) {
						...
						echo '<span class="title">';
						esc_html_e( 'Blog', 'kariez' );
						echo '</span>';
					} elseif ( class_exists( 'WooCommerce' ) and is_shop() ) {
						echo '<span class="title">';
						esc_html_e( 'Shop', 'kariez' );
						echo '</span>';
					}
					?>
				</li>
...
if ( ! function_exists( 'kariez_get_avatar_url' ) ) :
	function kariez_get_avatar_url( $get_avatar ) {
...
function kariez_comments_cbf( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback' : ?>
			<div class="pingback-entry"><span
					class="pingback-heading"><?php esc_html_e( 'Pingback:', 'kariez' ); ?></span> <?php comment_author_link(); ?>
			</div>
			<?php
			break;
		default :
			...
			<div class="comment-author">
				...
				<div class="author-info">
					<?php
					// Display author name
					printf( __( '<cite class="fn">%s</cite>', 'kariez' ), get_comment_author_link() ); ?>

					<div class="comment-meta commentmetadata">
						<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>"><?php
							/* translators: 1: date, 2: time */
							printf(
								__( '%1$s at %2$s', 'kariez' ),
								get_comment_date(),
								get_comment_time()
							); ?>
						</a><?php
						edit_comment_link( __( 'Edit', 'kariez' ), '  ', '' ); ?>
					</div><!-- .comment-meta -->
					<div class="comment-details">
						...
						if ( $comment->comment_approved == '0' ) { ?>
							<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'kariez' ); ?></em>
							<br/><?php
						} ?>

						<?php
						$icon = kariez_get_svg( 'share' );
						// Display comment reply link
						comment_reply_link( array_merge( $args, [
							'add_below'  => $add_below,
							'depth'      => $depth,
							'max_depth'  => $args['max_depth'],
							'reply_text' => $icon . __( 'Reply', 'kariez' )
						] ) ); ?>
...
if ( ! function_exists( 'kariez_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function kariez_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( kariez_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( kariez_option('rt_get_menu_label') ) ?></span>
				<?php } ?>

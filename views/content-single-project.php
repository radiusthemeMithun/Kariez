		...
		<div class="project-item-wrap">
			<div class="project-content-info sidebar-sticky">
				<div class="project-information">
					<?php if ( !empty( $rt_project_title ) && kariez_option( 'rt_project_title' )) { ?>
					...
					<?php } if ( !empty( $rt_project_text ) && kariez_option( 'rt_project_text' ) ) { ?>
						<p><?php echo esc_html( $rt_project_text );?></p>
					<?php } ?>
					<ul class="info-list">
						<?php if ( kariez_option( 'rt_project_cat' ) ) { ?>
							<li><label><?php esc_html_e( 'Category', 'kariez' );?>: </label>
						...
						<?php } ?>
						<?php if ( !empty( $rt_project_client ) && kariez_option( 'rt_project_client' ) ) { ?>
							<li><label><?php esc_html_e( 'Client', 'kariez' );?>: </label><?php echo esc_html( $rt_project_client );?></li>
						<?php } if ( !empty( $rt_project_start ) && kariez_option( 'rt_project_start' ) ) { ?>
							<li><label><?php esc_html_e( 'Starts On', 'kariez' );?>: </label><?php echo esc_html( $rt_project_start );?></li>
						<?php } if ( !empty( $rt_project_end ) && kariez_option( 'rt_project_end' ) ) { ?>
							<li><label><?php esc_html_e( 'Ends On', 'kariez' );?>: </label><?php echo esc_html( $rt_project_end );?></li>
						<?php } if ( !empty( $rt_project_weblink ) && kariez_option( 'rt_project_weblink' ) ) { ?>
							<li><label><?php esc_html_e( 'Web Link', 'kariez' );?>: </label><?php echo esc_html( $rt_project_weblink );?></li>
						<?php } ?>

						<?php if( kariez_option( 'rt_project_rating' ) ) { ?>
							<?php if( $ratting != -1) { ?>
								<li><label><?php esc_html_e( 'Rating', 'kariez' );?>: </label>
									<ul class="rating">
										<?php for ($i=0; $i < $ratting; $i++) { ?>
											<li class="star-rate"><i class="icon-star" aria-hidden="true"></i></li>
										<?php } ?>

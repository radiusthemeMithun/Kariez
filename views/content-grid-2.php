...
if ( kariez_option( 'rt_blog_above_meta_visibility' ) && !empty( has_post_thumbnail() ) && !empty( $rt_post_gallerys_raw ) ) {
	...
	<div class="article-inner-wrapper <?php echo esc_attr($wow . ' ' . $effect); ?>" data-wow-delay="<?php echo esc_attr($delay); ?>ms" data-wow-duration="<?php echo esc_attr($duration); ?>ms">
					...
					<div class="swiper-wrapper">
						<?php foreach( $rt_post_gallerys as $rt_posts_gallery ) { ?>
							<div class="swiper-slide">
								<?php echo wp_get_attachment_image( $rt_posts_gallery, 'kariez-size3', "", array( "class" => "img-responsive" ) );
								?>
							</div>
						<?php } ?>
					</div>
					<div class="swiper-navigation">
						<div class="swiper-button swiper-button-prev"><i class="icon-left-arrow"></i></div>
						<div class="swiper-button swiper-button-next"><i class="icon-right-arrow"></i></div>
					</div>
				</div>
			<?php } else { ?>
				<figure class="post-thumbnail">
					<a class="post-thumb-link alignwide" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1"><?php the_post_thumbnail( 'kariez-size3' ); ?></a>
				</figure><!-- .post-thumbnail -->
				<?php if ( kariez_option( 'rt_blog_above_meta_visibility' ) && !empty( has_post_thumbnail() )) { ?>
					<div class="posted-on">
				...
				if ( kariez_option( 'rt_video_visibility' ) == 1 && ( 'video' == get_post_format( get_the_ID() ) ) && !empty( $rt_youtube_link ) ) { ?>
					<div class="rt-video">
						<a class="popup-youtube video-popup-icon" href="<?php echo esc_url( $rt_youtube_link );?>">
							<i class="icon-play"></i>
						</a>
					</div>
				<?php } ?>
		...
		<div class="entry-wrapper">
			<header class="entry-header">
				<?php if ( ! empty( $meta_list ) && kariez_option( 'rt_meta_visibility' ) ) {
